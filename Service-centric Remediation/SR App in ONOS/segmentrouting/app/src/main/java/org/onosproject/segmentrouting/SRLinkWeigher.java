...
public final class SRLinkWeigher
    ...
    // threshold
    public static final ScalarWeight LINK_TO_AVOID_WEIGHT = new ScalarWeight(HOP_WEIGHT_VALUE + 100);//100
    ...
    public Weight weight(TopologyEdge edge) {
        ...
        //log.info("\n\n************** Edge: {}, Strip_Egde: {}, Size: {}\n\n", edge.toString(), edge.src().toString()+"-"+edge.dst().toString(), EcmpShortestPathGraph.link_Weight.size());
        float w = 0;
        if(EcmpShortestPathGraph.link_Weight.keySet().contains(edge.src().toString()+"-"+edge.dst().toString())){
            w = EcmpShortestPathGraph.link_Weight.get(edge.src().toString()+"-"+edge.dst().toString());
        }
        
        // All other cases we return
        return new ScalarWeight(w);
    }
