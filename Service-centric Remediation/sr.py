...
class IpHost(Host):
  ...
  def config(self, **kwargs):
    ...
    mtu = "ifconfig "+self.name+"-eth0 mtu 1490"#1490
    self.cmd(mtu)
...
class LeafSpine( Topo ):
  ...
  def __init__( self, spine, leaf, host ):
    ...
    for l in range(leaf):
      ...
      leafSwitch = self.addSwitch(name, dpid=dpid, protocols='OpenFlow13')
      ...
      for s in range(spine):
        ...
        #Add delay to link go through spine3
        #if s == 2:
        #  self.addLink(leafSwitch,spineSwitch,port1=s+1,port2=l+1,cls=TCLink,bw=10, delay='10ms')
        #else:
        #  self.addLink(leafSwitch,spineSwitch,port1=s+1,port2=l+1,cls=TCLink,bw=10)
        self.addLink(leafSwitch,spineSwitch,port1=s+1,port2=l+1,cls=TCLink,bw=10)
...
topos = { 'sr': ( lambda spine=5,leaf=2,host=2: LeafSpine(int(spine),int(leaf),int(host)) ) }
